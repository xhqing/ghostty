# FullStackEngineerAgent（Atlas）规则（fork 覆盖文件）

> 以下为 FullStackEngineerAgent（Atlas）CLAUDE.md 全文，其中「本项目」均指 FullStackEngineerAgent（不是本 fork 仓库）；「子项目」包含本仓库。来源：`~/Developer/FullStackEngineerAgent/CLAUDE.md`，权威源更新时同步覆盖此文件。
...
- **当前子项目清单**：zcode-cli（`~/Developer/zcode-cli`）、zcode-vsce（`~/Developer/zcode-vsce`）、ghostty-launcher（`~/Developer/ghostty-launcher`）、cmux-launcher（`~/Developer/cmux-launcher`）、pi（`~/Developer/pi`）、ghostty（`~/Developer/ghostty`，Ghostty fork，补丁分支 `paste-image`）。新增子项目时同步更新本清单。
